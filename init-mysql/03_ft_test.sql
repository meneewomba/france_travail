-- MySQL Script generated by MySQL Workbench
-- Sun May 25 07:58:59 2025
-- Model: New Model    Version: 1.0
-- MySQL Workbench Forward Engineering

SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=0;
SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0;
SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION';

-- -----------------------------------------------------
-- Schema ft_test
-- -----------------------------------------------------

-- -----------------------------------------------------
-- Schema ft_test
-- -----------------------------------------------------
CREATE DATABASE IF NOT EXISTS `ft_test` DEFAULT CHARACTER SET utf8 ;
USE `ft_test` ;

-- -----------------------------------------------------
-- Table `ft_test`.`naf`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ft_test`.`naf` ;

CREATE TABLE IF NOT EXISTS `ft_test`.`naf` (
  `naf_code` VARCHAR(10) NOT NULL,
  `label` VARCHAR(100) NULL DEFAULT NULL,
  PRIMARY KEY (`naf_code`),
  UNIQUE INDEX `label_UNIQUE` (`label` ASC) VISIBLE)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `ft_test`.`rome`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ft_test`.`rome` ;

CREATE TABLE IF NOT EXISTS `ft_test`.`rome` (
  `rome_code` VARCHAR(5) NOT NULL,
  `label` VARCHAR(45) NULL DEFAULT NULL,
  PRIMARY KEY (`rome_code`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `ft_test`.`activity_sector`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ft_test`.`activity_sector` ;

CREATE TABLE IF NOT EXISTS `ft_test`.`activity_sector` (
  `activity_sector_code` INT NOT NULL,
  `label` VARCHAR(100) NULL DEFAULT NULL,
  PRIMARY KEY (`activity_sector_code`),
  UNIQUE INDEX `label_UNIQUE` (`label` ASC) VISIBLE)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `ft_test`.`qualification`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ft_test`.`qualification` ;

CREATE TABLE IF NOT EXISTS `ft_test`.`qualification` (
  `qualification_code` INT NOT NULL,
  `label` VARCHAR(100) NULL DEFAULT NULL,
  PRIMARY KEY (`qualification_code`),
  UNIQUE INDEX `label_UNIQUE` (`label` ASC) VISIBLE)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `ft_test`.`companies`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ft_test`.`companies` ;

CREATE TABLE IF NOT EXISTS `ft_test`.`companies` (
  `company_id` INT NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(45) NULL DEFAULT NULL,
  `is_adapted` TINYINT NULL DEFAULT NULL,
  PRIMARY KEY (`company_id`),
  UNIQUE INDEX `name_UNIQUE` (`name` ASC) VISIBLE)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `ft_test`.`moving`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ft_test`.`moving` ;

CREATE TABLE IF NOT EXISTS `ft_test`.`moving` (
  `moving_code` TINYINT NOT NULL,
  `label` VARCHAR(45) NULL DEFAULT NULL,
  PRIMARY KEY (`moving_code`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `ft_test`.`cities`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ft_test`.`cities` ;

CREATE TABLE IF NOT EXISTS `ft_test`.`cities` (
  `insee_code` INT NOT NULL,
  `name` VARCHAR(45) NOT NULL,
  `arr` VARCHAR(45) NULL DEFAULT NULL,
  `latitude` VARCHAR(45) NULL DEFAULT NULL,
  `longitude` VARCHAR(45) NULL DEFAULT NULL,
  PRIMARY KEY (`insee_code`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `ft_test`.`requirements`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ft_test`.`requirements` ;

CREATE TABLE IF NOT EXISTS `ft_test`.`requirements` (
  `requirements` CHAR NOT NULL,
  `label` VARCHAR(45) NULL,
  PRIMARY KEY (`requirements`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `ft_test`.`job`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ft_test`.`job` ;

CREATE TABLE IF NOT EXISTS `ft_test`.`job` (
  `job_id` INT NOT NULL AUTO_INCREMENT,
  `title` VARCHAR(200) NOT NULL,
  `description` TEXT NOT NULL,
  `creation_date` TIMESTAMP NOT NULL,
  `update_date` TIMESTAMP NOT NULL,
  `rome_code` VARCHAR(5) NOT NULL,
  `experience_required` CHAR NOT NULL,
  `experience_length_months` INT NULL DEFAULT NULL,
  `experience_detail` VARCHAR(45) NULL DEFAULT NULL,
  `is_alternance` TINYINT NULL DEFAULT NULL,
  `is_disabled_friendly` TINYINT NULL DEFAULT NULL,
  `naf_code` VARCHAR(10) NULL DEFAULT NULL,
  `qualification_code` INT NULL DEFAULT NULL,
  `candidates_missing` TINYINT NULL DEFAULT NULL,
  `activity_sector_code` INT NULL DEFAULT NULL,
  `moving_code` TINYINT NULL DEFAULT NULL,
  `company_id` INT NULL DEFAULT NULL,
  `insee_code` INT NULL DEFAULT NULL,
  `internal_id` VARCHAR(12) NULL DEFAULT NULL,
  `active` TINYINT NULL,
  PRIMARY KEY (`job_id`),
  INDEX `naf_job_idx` (`naf_code` ASC) VISIBLE,
  INDEX `rome_job_fk_idx` (`rome_code` ASC) VISIBLE,
  INDEX `sector_activity_job_fk_idx` (`activity_sector_code` ASC) VISIBLE,
  INDEX `qualification_job_idx` (`qualification_code` ASC) VISIBLE,
  INDEX `job_company_fk_idx` (`company_id` ASC) VISIBLE,
  INDEX `job_moving_fk_idx` (`moving_code` ASC) VISIBLE,
  INDEX `insee_code_fk_idx` (`insee_code` ASC) VISIBLE,
  UNIQUE INDEX `internal_id_UNIQUE` (`internal_id` ASC) VISIBLE,
  INDEX `job_req_fk_idx` (`experience_required` ASC) VISIBLE,
  CONSTRAINT `naf_job_fk`
    FOREIGN KEY (`naf_code`)
    REFERENCES `ft_test`.`naf` (`naf_code`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `rome_job_fk`
    FOREIGN KEY (`rome_code`)
    REFERENCES `ft_test`.`rome` (`rome_code`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `sector_activity_job_fk`
    FOREIGN KEY (`activity_sector_code`)
    REFERENCES `ft_test`.`activity_sector` (`activity_sector_code`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `qualification_job`
    FOREIGN KEY (`qualification_code`)
    REFERENCES `ft_test`.`qualification` (`qualification_code`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `job_company_fk`
    FOREIGN KEY (`company_id`)
    REFERENCES `ft_test`.`companies` (`company_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `job_moving_fk`
    FOREIGN KEY (`moving_code`)
    REFERENCES `ft_test`.`moving` (`moving_code`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `insee_code_fk`
    FOREIGN KEY (`insee_code`)
    REFERENCES `ft_test`.`cities` (`insee_code`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `job_req_fk`
    FOREIGN KEY (`experience_required`)
    REFERENCES `ft_test`.`requirements` (`requirements`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `ft_test`.`salary`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ft_test`.`salary` ;

CREATE TABLE IF NOT EXISTS `ft_test`.`salary` (
  `salary_id` INT NOT NULL AUTO_INCREMENT,
  `job_id` INT NOT NULL,
  `min_monthly_salary` FLOAT NULL DEFAULT NULL,
  `max_monthly_salary` FLOAT NULL DEFAULT NULL,
  `salary_description` VARCHAR(100) NULL DEFAULT NULL,
  `min_monthly_predicted` FLOAT NULL,
  `min_salary_pred_diff` FLOAT NULL,
  `max_monthly_predicted` FLOAT NULL DEFAULT NULL,
  `max_salary_pred_diff` FLOAT NULL,
  `avg_min_diff` FLOAT NULL,
  `avg_max_diff` FLOAT NULL,
  PRIMARY KEY (`salary_id`),
  INDEX `job_id_idx` (`job_id` ASC) VISIBLE,
  UNIQUE INDEX `job_id_UNIQUE` (`job_id` ASC) VISIBLE,
  CONSTRAINT `salary_job_id_fk`
    FOREIGN KEY (`job_id`)
    REFERENCES `ft_test`.`job` (`job_id`)
    ON DELETE CASCADE
    ON UPDATE NO ACTION)
ENGINE = InnoDB
COMMENT = '	';


-- -----------------------------------------------------
-- Table `ft_test`.`benefits`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ft_test`.`benefits` ;

CREATE TABLE IF NOT EXISTS `ft_test`.`benefits` (
  `benefits_id` INT NOT NULL AUTO_INCREMENT,
  `label` VARCHAR(45) NULL DEFAULT NULL,
  PRIMARY KEY (`benefits_id`),
  UNIQUE INDEX `label_UNIQUE` (`label` ASC) VISIBLE)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `ft_test`.`salary_benefits`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ft_test`.`salary_benefits` ;

CREATE TABLE IF NOT EXISTS `ft_test`.`salary_benefits` (
  `benefits_id` INT NOT NULL,
  `salary_id` INT NOT NULL,
  PRIMARY KEY (`benefits_id`, `salary_id`),
  INDEX `salary_id_idx` (`salary_id` ASC) VISIBLE,
  CONSTRAINT `salary_benefits_benefits_id_fk`
    FOREIGN KEY (`benefits_id`)
    REFERENCES `ft_test`.`benefits` (`benefits_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `salary_benefits_salary_id_fk`
    FOREIGN KEY (`salary_id`)
    REFERENCES `ft_test`.`salary` (`salary_id`)
    ON DELETE CASCADE
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `ft_test`.`competencies`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ft_test`.`competencies` ;

CREATE TABLE IF NOT EXISTS `ft_test`.`competencies` (
  `competency_code` INT NOT NULL,
  `label` VARCHAR(300) NULL DEFAULT NULL,
  PRIMARY KEY (`competency_code`),
  UNIQUE INDEX `label_UNIQUE` (`label` ASC) VISIBLE)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `ft_test`.`job_competency`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ft_test`.`job_competency` ;

CREATE TABLE IF NOT EXISTS `ft_test`.`job_competency` (
  `competency_code` INT NOT NULL,
  `job_id` INT NOT NULL,
  `required` CHAR NULL DEFAULT NULL,
  PRIMARY KEY (`competency_code`, `job_id`),
  INDEX `job_competency_job_id_fk` (`job_id` ASC) VISIBLE,
  CONSTRAINT `job_competency_job_id_fk`
    FOREIGN KEY (`job_id`)
    REFERENCES `ft_test`.`job` (`job_id`)
    ON DELETE CASCADE
    ON UPDATE NO ACTION,
  CONSTRAINT `competencies_fk`
    FOREIGN KEY (`competency_code`)
    REFERENCES `ft_test`.`competencies` (`competency_code`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `ft_test`.`professional_qualities`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ft_test`.`professional_qualities` ;

CREATE TABLE IF NOT EXISTS `ft_test`.`professional_qualities` (
  `professional_quality_id` INT NOT NULL AUTO_INCREMENT,
  `label` VARCHAR(255) NULL DEFAULT NULL,
  `description` VARCHAR(255) NULL DEFAULT NULL,
  PRIMARY KEY (`professional_quality_id`),
  UNIQUE INDEX `label_UNIQUE` (`label` ASC) VISIBLE)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `ft_test`.`job_professional_qualities`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ft_test`.`job_professional_qualities` ;

CREATE TABLE IF NOT EXISTS `ft_test`.`job_professional_qualities` (
  `professional_quality_id` INT NOT NULL,
  `job_id` INT NOT NULL,
  PRIMARY KEY (`professional_quality_id`, `job_id`),
  INDEX `job_profesionnal_job_id_fk_idx` (`job_id` ASC) VISIBLE,
  CONSTRAINT `job_profesionnal_job_id_fk2`
    FOREIGN KEY (`job_id`)
    REFERENCES `ft_test`.`job` (`job_id`)
    ON DELETE CASCADE
    ON UPDATE NO ACTION,
  CONSTRAINT `pro_job_fk`
    FOREIGN KEY (`professional_quality_id`)
    REFERENCES `ft_test`.`professional_qualities` (`professional_quality_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `ft_test`.`driver_license`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ft_test`.`driver_license` ;

CREATE TABLE IF NOT EXISTS `ft_test`.`driver_license` (
  `driver_license_id` INT NOT NULL AUTO_INCREMENT,
  `label` VARCHAR(100) NULL DEFAULT NULL,
  PRIMARY KEY (`driver_license_id`),
  UNIQUE INDEX `label_UNIQUE` (`label` ASC) VISIBLE)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `ft_test`.`job_driver_license`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ft_test`.`job_driver_license` ;

CREATE TABLE IF NOT EXISTS `ft_test`.`job_driver_license` (
  `job_id` INT NOT NULL,
  `driver_license_id` INT NOT NULL,
  `requirement` CHAR NULL DEFAULT NULL,
  PRIMARY KEY (`driver_license_id`, `job_id`),
  INDEX `driver_license_id_idx` (`driver_license_id` ASC) VISIBLE,
  INDEX `job_driver_license_job_id` (`job_id` ASC) VISIBLE,
  INDEX `job_driver_req_idx` (`requirement` ASC) VISIBLE,
  CONSTRAINT `job_driver_license_job_id`
    FOREIGN KEY (`job_id`)
    REFERENCES `ft_test`.`job` (`job_id`)
    ON DELETE CASCADE
    ON UPDATE NO ACTION,
  CONSTRAINT `job_driver_license_driver_license_id`
    FOREIGN KEY (`driver_license_id`)
    REFERENCES `ft_test`.`driver_license` (`driver_license_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `job_driver_req`
    FOREIGN KEY (`requirement`)
    REFERENCES `ft_test`.`requirements` (`requirements`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `ft_test`.`formation`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ft_test`.`formation` ;

CREATE TABLE IF NOT EXISTS `ft_test`.`formation` (
  `formation_id` INT NOT NULL AUTO_INCREMENT,
  `label` VARCHAR(100) NULL DEFAULT NULL,
  `level` VARCHAR(100) NULL DEFAULT NULL,
  PRIMARY KEY (`formation_id`),
  UNIQUE INDEX `label_UNIQUE` (`label` ASC) VISIBLE)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `ft_test`.`job_formation`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ft_test`.`job_formation` ;

CREATE TABLE IF NOT EXISTS `ft_test`.`job_formation` (
  `job_id` INT NOT NULL,
  `formation_id` INT NOT NULL,
  `requirement` CHAR NULL DEFAULT NULL,
  PRIMARY KEY (`job_id`, `formation_id`),
  INDEX `formation_id_idx` (`formation_id` ASC) VISIBLE,
  INDEX `job_formation_req_idx` (`requirement` ASC) VISIBLE,
  CONSTRAINT `job_formation_job_id`
    FOREIGN KEY (`job_id`)
    REFERENCES `ft_test`.`job` (`job_id`)
    ON DELETE CASCADE
    ON UPDATE NO ACTION,
  CONSTRAINT `job_formation_formation_id`
    FOREIGN KEY (`formation_id`)
    REFERENCES `ft_test`.`formation` (`formation_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `job_formation_req`
    FOREIGN KEY (`requirement`)
    REFERENCES `ft_test`.`requirements` (`requirements`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `ft_test`.`language`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ft_test`.`language` ;

CREATE TABLE IF NOT EXISTS `ft_test`.`language` (
  `language_id` INT NOT NULL AUTO_INCREMENT,
  `label` VARCHAR(45) NULL DEFAULT NULL,
  PRIMARY KEY (`language_id`),
  UNIQUE INDEX `label_UNIQUE` (`label` ASC) VISIBLE)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `ft_test`.`job_language`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ft_test`.`job_language` ;

CREATE TABLE IF NOT EXISTS `ft_test`.`job_language` (
  `language_id` INT NOT NULL,
  `job_id` INT NOT NULL,
  `requirement` CHAR NULL DEFAULT NULL,
  PRIMARY KEY (`language_id`, `job_id`),
  INDEX `job_language_fk_idx` (`job_id` ASC) VISIBLE,
  INDEX `job_language_req_idx` (`requirement` ASC) VISIBLE,
  CONSTRAINT `job_language_fk`
    FOREIGN KEY (`job_id`)
    REFERENCES `ft_test`.`job` (`job_id`)
    ON DELETE CASCADE
    ON UPDATE NO ACTION,
  CONSTRAINT `language_fk`
    FOREIGN KEY (`language_id`)
    REFERENCES `ft_test`.`language` (`language_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `job_language_req`
    FOREIGN KEY (`requirement`)
    REFERENCES `ft_test`.`requirements` (`requirements`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `ft_test`.`contract_type`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ft_test`.`contract_type` ;

CREATE TABLE IF NOT EXISTS `ft_test`.`contract_type` (
  `contract_type_id` INT NOT NULL AUTO_INCREMENT,
  `contract_type` VARCHAR(45) NULL DEFAULT NULL,
  PRIMARY KEY (`contract_type_id`),
  UNIQUE INDEX `contract_type_UNIQUE` (`contract_type` ASC) VISIBLE)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `ft_test`.`contract_nature`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ft_test`.`contract_nature` ;

CREATE TABLE IF NOT EXISTS `ft_test`.`contract_nature` (
  `contract_nature_id` INT NOT NULL AUTO_INCREMENT,
  `contract_nature` VARCHAR(45) NULL DEFAULT NULL,
  PRIMARY KEY (`contract_nature_id`),
  UNIQUE INDEX `contract_nature_UNIQUE` (`contract_nature` ASC) VISIBLE)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `ft_test`.`job_contract`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ft_test`.`job_contract` ;

CREATE TABLE IF NOT EXISTS `ft_test`.`job_contract` (
  `job_id` INT NOT NULL,
  `contract_type_id` INT NOT NULL,
  `contract_nature_id` INT NOT NULL,
  `work_duration` FLOAT NULL DEFAULT NULL,
  `hours_per_week` FLOAT NULL DEFAULT NULL,
  `work_condition` VARCHAR(45) NULL DEFAULT NULL,
  `additional_condition` VARCHAR(45) NULL DEFAULT NULL,
  `partial` TINYINT NULL DEFAULT NULL,
  `label` VARCHAR(50) NULL,
  INDEX `job_contract_fk_idx` (`job_id` ASC) VISIBLE,
  INDEX `job_contract_nature_idx` (`contract_nature_id` ASC) VISIBLE,
  INDEX `job_contract_type_idx` (`contract_type_id` ASC) VISIBLE,
  PRIMARY KEY (`contract_type_id`, `contract_nature_id`, `job_id`),
  CONSTRAINT `job_contract_fk`
    FOREIGN KEY (`job_id`)
    REFERENCES `ft_test`.`job` (`job_id`)
    ON DELETE CASCADE
    ON UPDATE NO ACTION,
  CONSTRAINT `job_contract_nature_fk`
    FOREIGN KEY (`contract_nature_id`)
    REFERENCES `ft_test`.`contract_nature` (`contract_nature_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `job_contract_type_fk`
    FOREIGN KEY (`contract_type_id`)
    REFERENCES `ft_test`.`contract_type` (`contract_type_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `ft_test`.`offers_evolution`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ft_test`.`offers_evolution` ;

CREATE TABLE IF NOT EXISTS `ft_test`.`offers_evolution` (
  `date` DATE NOT NULL,
  `job_offers` INT NULL,
  `dept` INT NOT NULL,
  PRIMARY KEY (`date`, `dept`))
ENGINE = InnoDB;


SET SQL_MODE=@OLD_SQL_MODE;
SET FOREIGN_KEY_CHECKS=@OLD_FOREIGN_KEY_CHECKS;
SET UNIQUE_CHECKS=@OLD_UNIQUE_CHECKS;
